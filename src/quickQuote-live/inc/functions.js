...
function api_redirect_url(formID, json, defaultquotetype) {
    ...
    function analyticsEvent(category_name, ga_name) {
        var refID = $("#refID").val();
        //console.log(ga_name, refID)
        ga("send", "event", category_name, ga_name, refID);
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            analyticsEvent("Quote Request", ga_name);
            ...
            if (data.defaultquotetype == "ELIXIR") {
                ...
                if (response.premium1Year) {
                    var years1Premium =
                        "<tr><td>1 Year Fixed Premium:</td><td class='orange'>&pound;" +
                        addCommas(response.premium1Year.toFixed(2)) +
                        "</td></tr>";
                }

                if (response.premium2Year) {
                    var years2Premium =
                        "<tr><td>2 Year Fixed Premium:</td><td class='orange'>&pound;" +
                        addCommas(response.premium2Year.toFixed(2)) +
                        "</td></tr>";
                }
                ...
                $("#openElixirApplication").on("click", function(e) {
                    window.open(
                        "https://www.sportsinsurance4u.com/media/2166/elixir-career-ending-sports-insurance-application.pdf"
                    );
                });
            } else {
        ...
        error: function(response, json) {
            ...
            hj("formSubmitFailed");
            analyticsEvent("Error Response", ga_name);
            ...
            //console.log(response.responseJSON.IntroducerReference)

            // Check if certificate now valid || Check if introducer reference invalid
            if (
                typeof response.responseJSON == "undefined" ||
                response.responseJSON.IntroducerReference
            ) {
                notice(true, formID, error_response, "error");
            }

            // Returns validation to the field name that is not hidden or disabled.
...
function changeOptions(json) {
    //Set variable for family
    let familyElement = json.elem.substring(0, json.elem.length - 2);
    var defaultquotetype = $("[name=defaultquotetype]").val();
    var refID = $("#refID").val();

    if (json.elem == "StartDate") {
        dob = json.DateOfBirth;
        elem = "#DateOfBirth-GPregistration";
    } else if (familyElement == "dob_family") {
        dob = json.DateOfBirth;
        elem = "#" + json.elem + "-GPregistration";
    } else {
        dob = json.DateOfBirth;
        elem = "#" + json.elem + "-GPregistration";
    }
    ...
    if (dob && json.StartDate) {
        //disableElement('#DateOfBirth-GPregistration', false)
        // Populate dropdown with list of provinces
        // console.log("Valid make request");
        // console.log(
        //   gp_registration_api_url +
        //     "?startDate=" +
        //     json.StartDate +
        //     "&dob=" +
        //     dob +
            ...
            function(data) {
                ...
                if (data.length > 0) {
                    ...
                    $.each(data, function(key, i) {
                        ...
                        disableElement(elem, false);
                    });
                } else {
                    disableElement(elem, true);
                }
            }
        ).error(function(err) {
            disableElement(elem, true);
...
function displayOption(bool, element) {
    if (bool === false) {
        ...
        $(element + " input, " + element + " select").attr(
            "disabled",
            "disabled"
        );
    } else {
...
function brochureLink(type) {
    if (type == "PMI") {
        $("#brochureLink").attr(
            "href",
            "/generalandmedical/media/1451/private-health-insurance-brochure.pdf"
        );
    } else if (type == "SAI") {
        $("#brochureLink").attr(
            "href",
            "/sports-insurance-uk/media/1921/sports-accident-insurance-brochure.pdf"
        );
    } else if (type == "SHI") {
        $("#brochureLink").attr(
            "href",
            "/sports-insurance-uk/media/1913/sports-health-insurance-brochure.pdf"
        );
    } else if (type == "CHI") {
        $("#brochureLink").attr(
            "href",
            "/generalandmedical/media/1679/corporate-health-insurance-brochure.pdf"
        );
    } else if (type == "ELIXIR") {
        $("#brochureLink").attr(
            "href",
            "/sports-insurance-uk/media/2165/elixir-career-ending-sports-insurance-brochure.pdf"
        );
    }
}
...
function sportType(type) {
    ...
    $("#DateOfBirth-GPregistration").attr("disabled", true);
    ...
    if (type == "PMI" || type == "") {
        $("#quoteDescription").html(
            "Private Health Insurance for you and your family."
        );

        $("#familyGuidePrice").html(
            "Click 'Next' to see your guide price. You'll also be able to personalise your cover and add family members."
        );

        // Change button message
        $("#sportSubmit").html("Next <i class='fas fa-chevron-right'></i>");

        // GP registration for SAI only
        ...
        $("#quoteDescription").html(
            "Cash benefits for accidental injury and income protection cover, if you earn less than £10,000 a year from your sport."
        );

        $("#familyGuidePrice").html("");
        // Change button message
        $("#sportSubmit").html("See My Quotes");
        ...
        // GP registration for SAI only
        displayOption(false, "#DateOfBirth-GPregistration");
        displayOption(false, "#Spouse-GPregistration");
        ...
        $("#quoteDescription").html(
            "Private Health Insurance for you with important cover for sports injuries, for amateur and professional sportspeople."
        );
        $("#familyGuidePrice").html("");
        // Change button message
        $("#sportSubmit").html("See My Quotes");
        ...
        $("#quoteDescription").html(
            "Elixir offers cover and benefits for professional football players if they receive a career ending injury."
        );
        $("#familyGuidePrice").html("");
        // Change button message
        $("#sportSubmit").html("See My Quotes");
...
function dobFormat(json) {
    var data = JSON.parse(json);
    if (data.MainMember) {
        dateOfBirth = data.MainMember.DateOfBirth.split("-")
            .reverse()
            .join("-");
        data.MainMember.DateOfBirth = dateOfBirth;
    }
    if (data.DateOfBirth) {
        dateOfBirth = data.DateOfBirth.split("-")
            .reverse()
            .join("-");
        data.DateOfBirth = dateOfBirth;
    }
    ...
    // }
    if (data.addSpouse) {
        spouseDateOfBirth = data.Spouse.DateOfBirth.split("-")
            .reverse()
            .join("-");
    ...
    if (data.familyMembers) {
        ...
        data.familyMembers.forEach(function(i) {
            var integ = count++;
            familyDateOfBirth = i.dateOfBirth
                .split("-")
                .reverse()
                .join("-");
